...
//        pakai ini
            ...
            $reload = "listpendingdmcasermu.php?pagination=true&keyword=$keyword&username=$username&name=$name&data=$datatest";
			$sql="select * from tbl_acctmaintenancetemp where flagWF='dm' and ($strTable like '%$keyword%')";
	...
	//jika tidak ada pencarian pakai ini
            $reload = "listpendingdmcasermu.php?pagination=true&username=$username&name=$name&data=$datatest";
            //$sql =  "select * from tbl_acctmaintenancetemp";
			$sql =  "select * from tbl_acctmaintenancetemp where flagWF='dm'";
            $result = mysql_query($sql);
			$rwdt = mysql_fetch_array($result);
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-7">
                    ...
                    if($_REQUEST['keyword']<>""){
                    ?>
                        <a class="btn btn-default btn-outline" href="listpendingdmcasermu.php?username=<?php echo $username; ?>&name=<?php echo $name?>&data=<?php echo $datatest;?>"> Reset Pencarian</a>
                    <?php
                    }
                    ?>
                </div>
				...
				<hr/>
                    <form method="get" action="listpendingdmcasermu.php?username=<?php echo $username; ?>&name=<?php echo $name?>&data=<?php echo $datatest;?>">
						...
						<table>
							<tr>
								...
								<td><input type="text" name="keyword" class="form-control" value="<?php echo $_REQUEST['keyword']; ?>">
                                    ...
                                    <input type="hidden" name="routedata" value="toRMU">
                                    <input type="hidden" name="route" value="dm">
                                </td>
								<td>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
					...
					<tr>
						...
						<input type="hidden" name="name" value="<?php echo $name;?>">
						<input type="hidden" name="routedata" value="toRMU">
						<input type="hidden" name="route" value="dm"></td>
						<?php
						
							$querys = "select a.userId,b.firstName from dir_user_group a  join dir_user b on a.userId =b.username where a.groupId='groupDM'";
