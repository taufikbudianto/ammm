<?php 
	...
	if ($_GET['error']<>""){
		if($error=="sukses"){
			...
			echo '<script language="javascript">';
			echo 'alert("Generate Id Joget Gagal <Belum Pilih Assign To>")';
			echo '</script>';
			}
		}
...
//        pakai ini
            ...
            $reload = "proses.php?pagination=true&keyword=$keyword";
			$sql =  "select * from tbl_acctmaintenancetemp where flagCCBM ='Open' and ($strTable like '%$keyword%') and jnsLaporan not like '%Perubahan Data'";
	...
	//jika tidak ada pencarian pakai ini
            $reload = "proses.php?pagination=true";
            //$sql =  "select * from tbl_acctmaintenancetemp";
			$sql =  "select * from tbl_acctmaintenancetemp where flagCCBM ='Open' and jnsLaporan not like '%Perubahan Data'";//Taufik 16112016
            $result = mysql_query($sql);
			$rwdt = mysql_fetch_array($result);
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-7">
                    ...
                    if($_REQUEST['keyword']<>""){
                    ?>
                        <a class="btn btn-default btn-outline" href="proses.php"> Reset Pencarian</a>
                    <?php
                    }
                    ?>
                </div>
							...
							<tr>
                                <td>
									<select name="tablename" class="form-control" value="<?php echo $strTable; ?>">
										...
										<option value="custName">Case Type</option>
		 ...
		 <!-- <form method="post" action="http://10.11.88.234:8185/ServletWeb/servlet">-->
			    <form method="post" action="http://172.29.109.116:8080/save/mileage">
            <table class="table table-bordered">
                ...
                <tbody>
					...
					<tr>
						<td colspan="3">  <input type="hidden" name="tags[username]" value="<?php echo $username;?>">
						<input type="hidden" name="tags[name]" value="<?php echo $name;?>">
						<input type="hidden" name="tags[route]" value="am"></td>
						<td colspan ="2"><select name="tags[assignTo]" id="tags[assignTo]" class="form-control" onchange="showUser(this.value)">
							<option value="">Select</option>
							<option value="13117872">RINDANG PANGESTU (13117872)</option>
							<option value="14038312">VERANI ROSITA (14038312)</option>
							<option value="16012418">NURUL HUSNA (16012418)</option>
							<option value="16094627">GITA DEWI (16094627)</option>
						</select></td>
